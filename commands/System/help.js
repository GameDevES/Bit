...
module.exports = class extends Command {
	...
	async run(msg, [cmd]) {
		const gamedeves = msg.client.emojis.find("id", "366622024187117578");
		const nombreboteh = msg.client.emojis.find("id", "374651937557053442");
		const nombrebotconfundido = msg.client.emojis.find("id", "374652105623076865");
				...
				if (cmd) {
					if (cmd && help[categories[cat]][subCategories[subCat]].includes(cmd.name) && cmd.admins && !(cmd.name == 'ayuda')) {
						info = [
							...
							`${gamedeves}` + ' **Descripción:**   ' + cmd.description + '\n',
							`${gamedeves} **Ejemplo de uso:**   _`+ cmd.extendedHelp + `_\n`,
							'```md',
